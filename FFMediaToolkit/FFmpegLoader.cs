...
public static class FFmpegLoader
{
    ...
    public static unsafe void SetupLogging()
    {
        ...
        void Callback(void* p0, int level, string format, byte* vl)
        {
            if (level > ffmpeg.av_log_get_level()) return;

            const int LINE_SIZE = 1024;
            byte* lineBuffer = stackalloc byte[LINE_SIZE];
            int printPrefix = 1;
            ffmpeg.av_log_format_line(p0, level, format, vl, lineBuffer, LINE_SIZE, &printPrefix);
            string line = Marshal.PtrToStringAnsi((nint)lineBuffer);
            LogCallback?.Invoke(line);
        }

        ffmpeg.av_log_set_callback((av_log_set_callback_callback)Callback);
    }
