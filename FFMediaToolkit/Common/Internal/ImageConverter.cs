...
internal unsafe class ImageConverter : Wrapper<SwsContext>
{
    ...
    internal void FillAVFrame(ImageData bitmap, VideoFrame destinationFrame)
    {
        ...
        int requiredBufferLength = (bitmap.Stride * bitmap.ImageSize.Height) + BufferPaddingSize;
        bool shouldUseTmpBuffer = (bitmap.ImageSize != destinationSize) && (bitmap.Data.Length < requiredBufferLength);
        ...
        Span<byte> source = shouldUseTmpBuffer ? tmpBuffer : bitmap.Data;
        fixed (byte* ptr = source)
        {
            byte*[] data = new byte*[4] { ptr, null, null, null };
            int[] linesize = new int[4] { bitmap.Stride, 0, 0, 0 };
    ...
    internal void AVFrameToBitmap(VideoFrame videoFrame, byte* destination, int stride)
    {
        ...
        byte*[] data = new byte*[4] { destination, null, null, null };
        int[] linesize = new int[4] { stride, 0, 0, 0 };
    ...
    private void UpdateContext(Size sourceSize, AVPixelFormat sourceFormat)
    {
        if ((sourceSize != lastSourceSize) || (sourceFormat != lastSourcePixelFormat))
        {
            ...
            int scaleMode = sourceSize == destinationSize ? ffmpeg.SWS_POINT : ffmpeg.SWS_BICUBIC;
            SwsContext* swsContext = ffmpeg.sws_getContext(sourceSize.Width, sourceSize.Height, sourceFormat, destinationSize.Width, destinationSize.Height, destinationFormat, scaleMode, null, null, null);
