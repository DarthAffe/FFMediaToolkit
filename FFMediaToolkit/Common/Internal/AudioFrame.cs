...
internal unsafe class AudioFrame : MediaFrame
{
    ...
    public static AudioFrame CreateEmpty() => new();
    ...
    public float[][] GetSampleData()
    {
        ...
        for (uint ch = 0; ch < NumChannels; ch++)
        {
            ...
            Span<float> channelData = GetChannelData(ch);
            Span<float> sampleData = new(samples[ch], 0, NumSamples);
    ...
    public void UpdateChannelData(float[] samples, uint channel)
    {
        ...
        Span<float> frameData = GetChannelData(channel);
        Span<float> sampleData = new(samples, 0, NumSamples);
    ...
    public void UpdateFromSampleData(float[][] samples)
    {
        ...
        for (uint ch = 0; ch < NumChannels; ch++)
        {
            Span<float> newData = new(samples[ch], 0, NumSamples);
            Span<float> frameData = GetChannelData(ch);
    ...
    public void UpdateFromAudioData(AudioData audioData)
    {
        ...
        for (uint ch = 0; ch < NumChannels; ch++)
        {
            Span<float> newData = audioData.GetChannelData(ch);
            Span<float> currData = GetChannelData(ch);
