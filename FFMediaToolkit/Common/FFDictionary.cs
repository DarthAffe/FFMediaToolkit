...
internal unsafe class FFDictionary : Wrapper<AVDictionary>
{
    ...
    public static Dictionary<string, string> ToDictionary(AVDictionary* dictionary, bool ignoreSuffix = false)
    {
        ...
        AVDictionaryEntry* item = ffmpeg.av_dict_get(dictionary, string.Empty, null, ignoreSuffix ? ffmpeg.AV_DICT_IGNORE_SUFFIX : 0);

        while (item != null)
        {
            result[new nint(item->key).Utf8ToString()] = new nint(item->value).Utf8ToString();
    ...
    {
        AVDictionaryEntry* ptr = ffmpeg.av_dict_get(Pointer, key, null, matchCase ? ffmpeg.AV_DICT_MATCH_CASE : 0);
        return ptr != null ? new nint(ptr).Utf8ToString() : null;
    }
