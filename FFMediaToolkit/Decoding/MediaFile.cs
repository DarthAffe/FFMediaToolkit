...
public class MediaFile : IDisposable
{
    ...
    private unsafe MediaFile(InputContainer container, MediaOptions options)
    {
        ...
        IEnumerable<Decoder> video = container.Decoders.Where(codec => codec?.Info.Type == MediaType.Video);
        IEnumerable<Decoder> audio = container.Decoders.Where(codec => codec?.Info.Type == MediaType.Audio);
    ...
    public static MediaFile Open(string path, MediaOptions options)
    {
        try
        {
            InputContainer container = InputContainer.LoadFile(path, options);
    ...
    public static MediaFile Open(Stream stream, MediaOptions options)
    {
        try
        {
            InputContainer container = InputContainer.LoadStream(stream, options);
            return new MediaFile(container, options);
        }
        catch (Exception ex)
        {
    ...
    public void Dispose()
    {
        ...
        IEnumerable<MediaStream> video = VideoStreams.Cast<MediaStream>();
        IEnumerable<MediaStream> audio = AudioStreams.Cast<MediaStream>();

        IEnumerable<MediaStream> streams = video.Concat(audio);
