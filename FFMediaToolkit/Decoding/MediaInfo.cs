...
public class MediaInfo
{
    ...
    {
        FilePath = new nint(container->url).Utf8ToString();
        ContainerFormat = new nint(container->iformat->name).Utf8ToString();
        Metadata = new ContainerMetadata(container->metadata);
        Bitrate = container->bit_rate > 0 ? container->bit_rate : 0;

        AVRational timeBase = new()
    ...
    {
        MediaChapter[] streamChapters = new MediaChapter[container->nb_chapters];

        for (int i = 0; i < container->nb_chapters; i++)
        {
            AVChapter* chapter = container->chapters[i];
            AVDictionary* meta = chapter->metadata;
            TimeSpan startTimespan = chapter->start.ToTimeSpan(chapter->time_base);
            TimeSpan endTimespan = chapter->end.ToTimeSpan(chapter->time_base);
            streamChapters[i] =
