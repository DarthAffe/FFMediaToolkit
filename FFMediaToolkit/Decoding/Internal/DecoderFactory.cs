...
internal unsafe class DecoderFactory
{
    ...
    {
        AVFormatContext* format = container.Pointer;
        AVCodec* codec = null;

        int index = ffmpeg.av_find_best_stream(format, stream->codecpar->codec_type, stream->index, -1, &codec, 0);
        ...
        AVDictionary* dict = new FFDictionary(options.DecoderOptions, false).Pointer;
