...
internal static unsafe class OutputStreamFactory
{
    ...
    {
        AVCodecID codecId = config.Codec == VideoCodec.Default ? container.Pointer->oformat->video_codec : (AVCodecID)config.Codec;
        ...
        AVStream* stream = ffmpeg.avformat_new_stream(container.Pointer, codec);
        if (stream == null)
        ...
        FFDictionary dict = new(config.CodecOptions);
    ...
    {
        AVCodecID codecId = config.Codec == AudioCodec.Default ? container.Pointer->oformat->audio_codec : (AVCodecID)config.Codec;
