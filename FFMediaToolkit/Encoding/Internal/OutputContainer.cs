...
internal unsafe class OutputContainer : Wrapper<AVFormatContext>
{
    ...
    /// </summary>
    internal FFDictionary ContainerOptions { get; private set; } = new();
    ...
    public static OutputContainer Create(string extension)
    {
        ...
        AVOutputFormat* format = ffmpeg.av_guess_format(null, "x." + extension, null);
    ...
    public void CreateFile(string path)
    {
        ...
        AVDictionary* ptr = ContainerOptions.Pointer;
    ...
    {
        foreach ((OutputStream<VideoFrame> stream, VideoEncoderSettings config) output in Video)
        {
            output.stream.Dispose();
        }

        foreach ((OutputStream<AudioFrame> stream, AudioEncoderSettings config) output in Audio)
        {
            output.stream.Dispose();
        }
